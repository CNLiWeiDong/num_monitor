...
namespace hb {
    namespace plugin {
        ...
        void monitor_price_plugin_impl::loop() {
            ...
            deadline_query_ = make_shared<boost::asio::deadline_timer>(
                io, boost::posix_time::seconds(intervals_seconds_));
            ...
            deadline_query_->async_wait([self, &io](const boost::system::error_code &ec) {
                ...
                self->deadline_updater_ = make_shared<boost::asio::deadline_timer>(
                    io, boost::posix_time::seconds(self->delay_update_seconds_)
                            + boost::posix_time::millisec(100));
                self->deadline_updater_->async_wait(
                    [self](const boost::system::error_code &ec) { self->deal_monitor(); });
                self->loop();
            });
        }
