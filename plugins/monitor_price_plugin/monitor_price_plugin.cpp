...
namespace hb {
    namespace plugin {
        static appbase::abstract_plugin& _monitor_price_plugin
            = app().register_plugin<monitor_price_plugin>();
        ...
        void monitor_price_plugin::set_program_options(options_description& cli,
            ...
            cfg.add_options()("monitor-intervals-seconds",
                ...
                "monitor-max-request-error", boost::program_options::value<int>()->default_value(5),
                "the max continuous request response error times.")(
                "monitor-target-config",
                boost::program_options::value<string>()->default_value("monitor.ini"),
                "monitor targets config file name.")(
                "monitor-request-error-mail-to",
                boost::program_options::value<string>()->default_value("357879926@qq.com"),
                "monitor targets request error, send mail to.");
        }
        void monitor_price_plugin::plugin_initialize(const variables_map& options) {
            ...
            for (int i = 1; i <= max_targets_num; i++) {
                ...
                LOG_INFO("add target:%s,%s,%s,%s", one_target.id.c_str(), one_target.host.c_str(),
                         one_target.format_value.c_str(), one_target.format_date.c_str());
