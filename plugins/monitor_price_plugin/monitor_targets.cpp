...
namespace hb {
    namespace plugin {
            ...
            if (item.request_error_times >= max_request_error_time_) {
                ...
                if (cur_seconds - item.last_send_error_time >= senderror_seconds_) {
                    ...
                    mail_plugin.send_mail(
                        request_error_mail_to_, subject.str(),
        ...
        void monitor_targets::set_target(target_type &item, const int &status,
            ...
            hb_try {
                LOG_INFO("request callback %s, %s, %s", item.id.c_str(), item.server_type.c_str(),
                         item.host.c_str());
                if (status != 200) {
                ...
                if (item.format_contents != "") {
                    ...
                    if (regex_value(reg, what, res)) {
                        ...
                        LOG_INFO("set target contents info %s, %u", item.id.c_str(),
                                 item.contents.size());
                    } else {
            ...
            hb_try {
                ...
                for (auto &it : all_targets_list_) {
                    ...
                    if (item.server_type == "https") {
                        hb::https::https hs(item.host, item.port, item.target,
                                            delay_update_seconds_);
