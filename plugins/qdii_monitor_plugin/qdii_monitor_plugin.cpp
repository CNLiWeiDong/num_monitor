...
#include <hb/qdii_monitor_plugin/qdii_monitor_plugin.h>
#include <stdlib.h>
...
namespace hb {
    namespace plugin {
        static appbase::abstract_plugin& _qdii_monitor_plugin
            = app().register_plugin<qdii_monitor_plugin>();
        qdii_monitor_plugin::qdii_monitor_plugin() {}
        qdii_monitor_plugin::~qdii_monitor_plugin() {}
        void qdii_monitor_plugin::set_program_options(options_description& cli,
                                                       options_description& cfg) {
            cfg.add_options()("qdii-intervals-seconds",
                              boost::program_options::value<int>()->default_value(3 * 60),
                              "the intervals seconds of update all targets info.")(
                "qdii-http-request-expire",
                ...
                /*
                    ...
                    1小时45分一次 9:30,11:15,13:00,14:45 = (1*60+45)*60
                    3小时31分一次 9:30,13:01 = (3*60+31)*60 *默认*
                */
                boost::program_options::value<int>()->default_value((3*60+31)*60),
                ...
                "qdii-max-request-error", boost::program_options::value<int>()->default_value(5),
                "the max continuous request response error times.")(
                "qdii-target-config",
                boost::program_options::value<string>()->default_value("qdii.ini"),
                "monitor targets config file name.");
        }
        void qdii_monitor_plugin::plugin_initialize(const variables_map& options) {
            log_info("qdii_monitor_plugin::plugin_initialize");
            my = make_shared<qdii_monitor_plugin_impl>();
            my->intervals_seconds(options.at("qdii-intervals-seconds").as<int>());
            my->send_target_info_seconds(
                options.at("qdii-send-target-info-intervals").as<int>());
            my->sendmsg_seconds(options.at("qdii-sendmsg-intervals").as<int>());
            my->senderror_seconds(options.at("qdii-senderror-intervals").as<int>());
            my->http_request_expire(options.at("qdii-http-request-expire").as<int>());
            my->max_request_error_time(options.at("qdii-max-request-error").as<int>());

            bfs::path config_file = options.at("qdii-target-config").as<string>();
            ...
            // set http info
            auto http_info = config_pt.get_child_optional("http_server");
            my->targets()->server_type_ = http_info->get<string>("server_type");
            my->targets()->host_ = http_info->get<string>("host");
            my->targets()->port_ = http_info->get<string>("port");
            my->targets()->target_ = http_info->get<string>("target");
            ...
            for (int i = 1; i <= max_targets_num; i++) {
                ...
                target_type one_target
                    ...
                    //    .format_value_index = target->get<int>("format_value_index"),
                       .format_date = target->get<string>("format_date", ""),
                    //    .format_date_index = target->get<int>("format_date_index"),
                ...
                LOG_INFO("add target:%s,%s", one_target.id.c_str(), one_target.name.c_str());
            ...
            for (int i = 1; i <= max_targets_num; i++) {
                ...
                LOG_INFO("add single:%s,%f,%f,%s,%s", id.c_str(), single.min, single.max,
                         single.phone.c_str(), single.mail.c_str());
            ...
            for (int i = 1; i <= max_targets_num; i++) {
                ...
                if (target_ids.find(id1) == target_ids.end()
                    || target_ids.find(id2) == target_ids.end()) {
                    hb::plugin::qdii_monitor_error e;
                    e.msg("pairs %s,%s is not existed in targets!", id1, id2);
                    hb_throw(e);
                }
                pairs_type p = {.id1 = id1,
                                .id2 = id2,
                                .min = target->get<double>("min"),
                                .max = target->get<double>("max"),
                                .phone = target->get<string>("phone"),
                                ...
                                .send_msg_time = 0,
                                .active = target->get<bool>("active")};
                LOG_INFO("add pair:%s,%s,%f,%f,%s", id1.c_str(), id2.c_str(), p.min, p.max,
                         p.mail.c_str());
                my->add_pairs(p);
            }
        }
        void qdii_monitor_plugin::plugin_startup() {
            log_info("qdii_monitor_plugin::plugin_startup");
            my->start();
        }
        void qdii_monitor_plugin::plugin_shutdown() {
            log_info("qdii_monitor_plugin::plugin_shutdown");
            if (my) my.reset();
        }
    }  // namespace plugin
}  // namespace hb
...
/*
                ...
                {
                    auto targets =
   options.at("monitor-all-targets-list").as<std::vector<std::string>>(); for(auto& arg : targets)
                    {
                        vector<string> props;
                        ...
                        if(week_vals.size()<2){
                            hb_throw("monitor-all-targets-list min and max week config is error:
   %s", props[5].c_str());
                        }
                        vector<string> minutes_vals;
                        boost::split(minutes_vals, props[6], boost::is_any_of("~:"));
                        if(minutes_vals.size()<2){
                            hb_throw("monitor-all-targets-list min and max hours config is error:
   %s", props[6].c_str());
                        }
                        target_type one_target = {
                            .id = props[0],
                        ...
                        LOG_INFO("add
   target:%s,%s,%s,%s,%s,%s,%s,%s",props[0].c_str(),props[1].c_str(),props[2].c_str(),props[3].c_str(),props[4].c_str(),props[5].c_str(),props[6].c_str(),props[7].c_str());
...
//         if(vals.size()<2){
//             hb_throw("monitor-singles-list min and max value config is error: %s",
//             props[1].c_str());
//         }
//         singles_type single = {
//             id: props[0],
...
//         };
//         LOG_INFO("add
//         single:%s,%s,%s,%s",props[0].c_str(),vals[0].c_str(),vals[1].c_str(),props[2].c_str());
//         my->add_single(single);
//     }
// }
...
//         }
//         if(target_ids.find(props[0])==target_ids.end() ||
//         target_ids.find(props[1])==target_ids.end()) {
//             hb_throw("pairs %s,%s is not existed in targets!", props[0].c_str(),
//             props[1].c_str());
//         }
//         vector<string> vals;
//         boost::split(vals, props[2], boost::is_any_of("~:"));
//         if(vals.size()<2){
//             hb_throw("monitor-pairs-list min and max value config is error: %s",
//             props[1].c_str());
//         }
//         pairs_type p = {
//             .id1 = props[0],
...
//         };
//         LOG_INFO("add
//         pair:%s,%s,%s,%s,%s",props[0].c_str(),props[1].c_str(),vals[0].c_str(),vals[1].c_str(),props[3].c_str());
//         my->add_pairs(p);
//     }
// }